...
export default function OnboardingTour({ onClose, activeStep, setActiveStep, setActiveMobileTab }: OnboardingTourProps) {
  ...
  const steps = [
    {
      ...
      text: "Я ваш гид Фини. Я помогу вам освоить это интерактивное приложение. Вы узнаете, как устроен бюджет Москвы, и научитесь получать ценные баллы, которые можно переводить в экосистемы «Миллион призов» и «Активный Гражданин»!",
      targetSelector: null,
      mood: "waving" as const,
    },
    {
      title: "Ваш профиль и баланс баллов",
      text: "В шапке находится ваш подтвержденный профиль Mos.ID. Здесь отображается ваш текущий уровень, накопленный опыт (XP) и баланс баллов. Зарабатывайте баллы в приложении и обменивайте их на реальные призы в экосистемах города!",
      targetSelector: "#tour-header",
      mood: "neutral" as const,
    },
    {
      title: "Чекап профиля и выбор роли",
      text: "Выберите свой социальный статус: Студент, Молодой специалист, Предприниматель или Семья с детьми. Настройки приложения, доступные льготы и налоговые треки подстроятся специально под вас.",
    ...
    },
    {
      ...
      text: "Выполняйте задания, проходите викторины по финансовой грамотности, играйте в симулятор Мэра района и исследуйте карту Москвы. Каждое достижение пополняет ваш баланс реальными баллами!",
      targetSelector: "#tour-quests",
      mood: "neutral" as const,
    },
    {
      title: "Интерактивная аналитика бюджета",
      text: "Исследуйте наглядную структуру расходов Москвы на 2026 год. Переключите режим в «Мой вклад», чтобы увидеть точное распределение вашего НДФЛ по программам развития города!",
      targetSelector: "#tour-analytics",
      mood: "thinking" as const,
    },
    {
      title: "Умный Бюджетный ИИ-Ассистент",
      text: "Если у вас возникнут сложные вопросы о бюджете Москвы, просто кликните сюда! ИИ-ассистент готов ответить на любые вопросы и запустить персональный экспресс-квиз с дополнительной наградой.",
      targetSelector: "#tour-ai",
      mood: "waving" as const,
    },
    {
      title: "Вы готовы исследовать бюджет!",
      text: "Отличная работа! Экскурсия завершена. Теперь вы полностью готовы управлять финансами, проходить квесты и зарабатывать призы. Удачи в изучении Открытого бюджета Москвы!",
      targetSelector: null,
      mood: "happy" as const,
    }
  ];
  ...
  const handleNext = () => {
    if (activeStep < steps.length - 1) {
      ...
      localStorage.setItem('mos_onboarding_completed_v3', 'true');
      onClose();
    }
  };

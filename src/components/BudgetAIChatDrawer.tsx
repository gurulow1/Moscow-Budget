...
const generateResponse = (query: string): string => {
  ...
  if (s.includes('викторин') || s.includes('тест') || s.includes('игра') || s.includes('вызов')) {
    return `Откройте раздел «Викторины», чтобы запустить квиз из проверенного банка вопросов.

Баллы в проекте учебные и отмечают только локальный прогресс.`;
  }
  ...
  if (
    ...
    return `На 2026 год доходы Москвы запланированы в размере **5 937,4 млрд ₽**, расходы — **6 385,0 млрд ₽**, дефицит — **447,6 млрд ₽**.
...
Общий лимит расходов для большинства социальных вычетов составляет **150 000 ₽**, а на обучение ребёнка — **110 000 ₽** на обоих родителей. Фактический возврат зависит от уплаченного НДФЛ. Источники: Закон Москвы № 39 и ФНС России.`;
  ...
  if (
    ...
    return `Развитие транспортной системы — одно из крупнейших направлений расходов Москвы. В учебной диаграмме оно показано округлённой долей около **20%**. Точные статьи и актуальное исполнение проверяйте на **budget.mos.ru**.`;
  }
  ...
  if (
    ...
    return `На социальную сферу Москвы в 2026 году предусмотрено около **3,2 трлн ₽**, то есть примерно половина расходов. Программа социальной поддержки жителей составляет **810 млрд ₽**. Источник: портал «Открытый бюджет Москвы».`;
  }
  ...
  if (
    ...
    return `На развитие образования Москвы в 2026 году предусмотрено **814,6 млрд ₽** — около 12,8% всех плановых расходов. Детализацию программы смотрите на **budget.mos.ru**.`;
  }
  ...
  if (
    ...
    return `На развитие здравоохранения Москвы в 2026 году предусмотрено **615 млрд ₽** без учёта оплаты медицинской помощи из Фонда ОМС. Детализацию программы проверяйте на **budget.mos.ru**.`;
  }
  ...
  if (
    ...
    return `В плане на 2026 год на программу **«Экономическое развитие и инвестиционная привлекательность»** предусмотрено **226,5 млрд ₽**, а на развитие цифровой среды и инноваций — **243,1 млрд ₽**. Условия конкретных льгот нужно проверять на официальных страницах программ.`;
  }

  return `Это локальный интерактивный справочник, а не официальный консультант и не генеративный ИИ. Доступны проверенные верхнеуровневые показатели по темам **доходы**, **социальная сфера**, **образование**, **здравоохранение**, **транспорт** и **экономика**. Первичные данные: **budget.mos.ru**; налоговые вычеты: **nalog.gov.ru**.`;
};
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-[rgba(148,163,184,0.15)] bg-[#151f32]/90">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-xs font-extrabold text-[#f8fafc] leading-tight">Бюджетный справочник</h3>
                    <span className="text-[10px] text-[#94a3b8] font-bold uppercase tracking-wider block">Подготовленная база ответов</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <button
